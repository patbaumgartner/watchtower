...
import (
	...
	"github.com/patbaumgartner/watchtower/pkg/registry/auth"

	ref "github.com/distribution/reference"
	. "github.com/onsi/ginkgo"
	. "github.com/onsi/gomega"
	wtTypes "github.com/patbaumgartner/watchtower/pkg/types"
)
...
var _ = Describe("the auth module", func() {
	...
	Describe("GetAuthURL", func() {
		It("should create a valid auth url object based on the challenge header supplied", func() {
			...
			expected := &url.URL{
				...
				Scheme:   "https",
				Path:     "/token",
				RawQuery: "scope=repository%3Apatbaumgartner%2Fwatchtower%3Apull&service=ghcr.io",
			}
		...
		When("deriving the auth scope from an image name", func() {
			...
			It("should not include vanity hosts\"", func() {
				Expect(getScopeFromImageAuthURL("docker.io/patbaumgartner/watchtower")).To(Equal("patbaumgartner/watchtower"))
				Expect(getScopeFromImageAuthURL("index.docker.io/patbaumgartner/watchtower")).To(Equal("patbaumgartner/watchtower"))
			})
			It("should not destroy three segment image names\"", func() {
				Expect(getScopeFromImageAuthURL("piksel/patbaumgartner/watchtower")).To(Equal("piksel/patbaumgartner/watchtower"))
				Expect(getScopeFromImageAuthURL("ghcr.io/piksel/patbaumgartner/watchtower")).To(Equal("piksel/patbaumgartner/watchtower"))
			})
			It("should not prepend library/ to image names if they're not on dockerhub", func() {
				...
				Expect(getScopeFromImageAuthURL("ghcr.io/patbaumgartner/watchtower")).To(Equal("patbaumgartner/watchtower"))
			})
		})
	...
	Describe("GetChallengeURL", func() {
		It("should create a valid challenge url object based on the image ref supplied", func() {
			...
			imageRef, _ := ref.ParseNormalizedNamed("ghcr.io/patbaumgartner/watchtower:latest")
		...
		It("should assume Docker Hub for image refs with no explicit registry", func() {
			...
			imageRef, _ := ref.ParseNormalizedNamed("patbaumgartner/watchtower:latest")
			Expect(auth.GetChallengeURL(imageRef)).To(Equal(expected))
		})
		It("should use index.docker.io if the image ref specifies docker.io", func() {
			expected := url.URL{Host: "index.docker.io", Scheme: "https", Path: "/v2/"}
			imageRef, _ := ref.ParseNormalizedNamed("docker.io/patbaumgartner/watchtower:latest")
			Expect(auth.GetChallengeURL(imageRef)).To(Equal(expected))
		})
	})
})
